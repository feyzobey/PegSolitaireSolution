...
def print_board_ascii(state_tuple):
    ...
    for r in range(7):
        ...
        for c in range(7):
            idx = grid_map[r][c]
            if idx == 0: row += " "
            else: row += "X" if state_tuple[idx] == 1 else "O"
        print(row)
    print("-" * 10)
...
def main():
    ...
    if status == "TIMEOUT":
        ...
        if status == "SUCCESS": print("Goal reached: one peg in the center (31 moves).")
        else: 
    ...
    if node:
        ...
        for action, state in path:
            if action: print(f"Move: {action[0]}-{action[1]}-{action[2]}") # from-over-to
